...
float r0_MQ5 = 20.0; // Ajustado según tu lectura de ~180k en aire limpia (180/9)
...
void getConfig() {
  ...
  if (http.begin(client, configUrl)) {
    ...
    if (httpCode == 200) {
      ...
      Serial.printf("[CONFIG] Umbrales cargados -> MQ2:%.0f MQ3:%.0f MQ5:%.0f MQ9:%.0f\n",
...
void streamSensorData() {
  ...
  SensorReading mq3 = readMQSensor(MQ3_PIN, r0_MQ3, 200.0, -1.4);   // +sensible (antes 150, -1.5)
  SensorReading mq5 = readMQSensor(MQ5_PIN, r0_MQ5, 500.0, -2.5);
  SensorReading mq9 = readMQSensor(MQ9_PIN, r0_MQ9, 800.0, -2.0);   // +sensible (antes 600, -2.2)
  ...
  if (maxP >= 2.0) maxSeverity = "CRITICAL";
  else if (maxP >= 1.5) maxSeverity = "HIGH";
  else if (maxP >= 1.0) maxSeverity = "MEDIUM";
  else if (maxP >= 0.8) maxSeverity = "LOW";
  ...
  bool anySensorAboveThreshold = (mq2.ppm >= THRESHOLD_MQ2 || mq3.ppm >= THRESHOLD_MQ3 || 
  ...
  if (autoMode) {
    // Histeresis: Si ya estaba prendido, se apaga solo si baja del 80% del umbral
    float margin = 0.8;
    bool danger = anySensorAboveThreshold;
    
    bool keepOn = (currentWindowOpen && (mq2.ppm >= THRESHOLD_MQ2*margin || mq3.ppm >= THRESHOLD_MQ3*margin || 
                                         mq5.ppm >= THRESHOLD_MQ5*margin || mq9.ppm >= THRESHOLD_MQ9*margin));
  ...
  // para que el frontend reciba datos en tiempo real sin importar si las alertas tardan
  if (wsConnected) {
    JsonDocument doc;
  ...
  auto shouldAlert = [&](float currentPpm, float lastPpm, unsigned long lastTime, float threshold) {
    if (currentPpm < threshold) return false;
    if (lastTime == 0) return true;
    unsigned long timePassed = now - lastTime;
    if (timePassed >= ALERT_COOLDOWN) {
        return (abs(currentPpm - lastPpm) > (lastPpm * 0.1) || currentPpm >= threshold);
    }
    return false;
  };
  ...
  if (!alertSent && shouldAlert(mq2.ppm, lastSentPpm_MQ2, lastAlertTime_MQ2, THRESHOLD_MQ2)) {
      lastAlertTime_MQ2 = now; lastSentPpm_MQ2 = mq2.ppm;
      sendGasAlert((mq2.ppm > 500) ? "SMOKE" : "LPG", mq2.ppm, mq2.voltage, mq2.raw);
      alertSent = true;
  }
  yield();
  if (!alertSent && shouldAlert(mq3.ppm, lastSentPpm_MQ3, lastAlertTime_MQ3, THRESHOLD_MQ3)) {
      lastAlertTime_MQ3 = now; lastSentPpm_MQ3 = mq3.ppm;
      sendGasAlert("ALCOHOL", mq3.ppm, mq3.voltage, mq3.raw);
      alertSent = true;
  }
  yield();
  if (!alertSent && shouldAlert(mq5.ppm, lastSentPpm_MQ5, lastAlertTime_MQ5, THRESHOLD_MQ5)) {
      lastAlertTime_MQ5 = now; lastSentPpm_MQ5 = mq5.ppm;
      sendGasAlert("METHANE", mq5.ppm, mq5.voltage, mq5.raw);
      alertSent = true;
  }
  yield();
  if (!alertSent && shouldAlert(mq9.ppm, lastSentPpm_MQ9, lastAlertTime_MQ9, THRESHOLD_MQ9)) {
      lastAlertTime_MQ9 = now; lastSentPpm_MQ9 = mq9.ppm;
      sendGasAlert((mq9.ppm > 150.0) ? "SMOKE" : "CO", mq9.ppm, mq9.voltage, mq9.raw);
      alertSent = true;
  }
}
  ...
  switch(type) {
    ...
    case sIOtype_EVENT: {
      ...
      if (event.indexOf("actuatorCommand") != -1) {
        ...
        // El formato es ["actuatorCommand", {"actuator": "window", "status": true}]
        JsonObject data = doc[1];
        String actuator = data["actuator"];
        bool status = data["status"];
        
        autoMode = false; // Desactivar modo auto temporalmente por intervención manual
...
void setup() {
  ...
  socketIO.begin(SERVER_HOSTNAME, SERVER_PORT, "/socket.io/?EIO=4&transport=websocket");
